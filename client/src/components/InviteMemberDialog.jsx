...
const InviteMemberDialog = ({ isDialogOpen, setIsDialogOpen }) => {
  ...
  const [formData, setFormData] = useState({
    email: "",
    role: "MEMBER", // Prisma expects ADMIN or MEMBER
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Send request to backend to add workspace member
      await api.post(
      ...
      // Refresh workspaces so UI shows new member immediately
      const wsRes = await api.get("/api/workspaces", {
        headers: { Authorization: `Bearer ${token}` },
      });

      dispatch(setWorkspaces(wsRes.data.workspaces));
  ...
  return (
    <div className="fixed inset-0 bg-black/20 dark:bg-black/50 backdrop-blur flex items-center justify-center z-50">
      <div className="bg-white dark:bg-zinc-950 border border-zinc-300 dark:border-zinc-800 rounded-xl p-6 w-full max-w-md text-zinc-900 dark:text-zinc-200">
        <div className="mb-4">
          <h2 className="text-xl font-bold flex items-center gap-2">
            <UserPlus className="size-5 text-zinc-900 dark:text-zinc-200" /> Invite
            Team Member
          </h2>
          {currentWorkspace && (
            <p className="text-sm text-zinc-700 dark:text-zinc-400">
