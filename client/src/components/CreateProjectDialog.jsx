...
const CreateProjectDialog = ({ isDialogOpen, setIsDialogOpen }) => {
  ...
  // helper to safely get member email (depends on your backend shape)
  const memberEmail = (member) => member?.user?.email ?? member?.email ?? "";
            ...
            <select
              ...
              onChange={(e) =>
                setFormData((prev) => {
                  ...
                  const membersSet = new Set(prev.team_members);
                  if (lead) membersSet.add(lead);
                  return { ...prev, team_lead: lead, team_members: Array.from(membersSet) };
                })
              }
              className="w-full px-3 py-2 rounded dark:bg-zinc-900 border border-zinc-300 dark:border-zinc-700 mt-1 text-zinc-900 dark:text-zinc-200 text-sm"
            >
          ...
          <div>
            ...
            >
              ...
              {currentWorkspace?.members
                ?.filter((m) => !formData.team_members.includes(memberEmail(m)))
                .map((member) => {
                  const email = memberEmail(member);
                  return (
                    <option key={email} value={email}>
